...
export default function DashboardPage() {
  // Détecter si on est dans une popup en mode "ajouter un compte" après OAuth
  useEffect(() => {
    ...
    const handleOAuthCallback = async () => {
      ...
      console.log("Dashboard: Checking OAuth callback", { 
        isAdding, 
      ...
      if (isAdding) {
        ...
        if (session?.data?.user) {
          ...
          // Déterminer le provider depuis l'URL, le referrer, ou la session
          let provider = "unknown";
          
          // Vérifier l'URL actuelle
          if (window.location.href.includes("github") || window.location.href.includes("github.com")) {
            provider = "github";
          } else if (window.location.href.includes("google") || window.location.href.includes("google.com") || window.location.href.includes("accounts.google.com")) {
            provider = "google";
          }
          // Vérifier le referrer
          else if (document.referrer.includes("github") || document.referrer.includes("github.com")) {
            provider = "github";
          } else if (document.referrer.includes("google") || document.referrer.includes("google.com") || document.referrer.includes("accounts.google.com")) {
            provider = "google";
          }
          // Vérifier dans sessionStorage (peut être défini lors de l'ouverture de la popup)
          // Toujours vérifier sessionStorage en priorité car il est plus fiable
          const storedProvider = sessionStorage.getItem("oauthProvider");
          if (storedProvider) {
            provider = storedProvider;
            console.log("Dashboard: Using provider from sessionStorage", provider);
            sessionStorage.removeItem("oauthProvider");
          }
          
          // Si toujours unknown, essayer de détecter depuis l'URL ou le referrer
          if (provider === "unknown") {
            // Vérifier l'URL actuelle
            if (window.location.href.includes("github") || window.location.href.includes("github.com")) {
              provider = "github";
          ...
          console.log("Dashboard: OAuth callback detected, sending account to parent:", { 
            email: user.email, 
            name: user.name,
            provider,
            hasOpener: !!window.opener
          ...
          // Envoyer le message à la fenêtre parente plusieurs fois pour s'assurer qu'il est reçu
          const sendMessage = () => {
            if (window.opener && !window.closed) {
              ...
              console.log("Dashboard: Message sent to parent");
            }
          };
          ...
          // Envoyer à nouveau après un court délai
          ...
          setTimeout(() => {
            if (window.opener && !window.closed) {
              console.log("Dashboard: Closing popup");
              window.close();
            }
          }, 1500);
        } else {
          console.error("Dashboard: No session found after OAuth callback");
          sessionStorage.removeItem("addingAccount");
          if (window.opener) {
            window.close();
          }
  ...
  return (
    <AppContainer>
      <Header />
      <DashboardContent />
      <Button icon="add-circle-bold">
        Add Project
      </Button>
      <Button variant="accent" icon="add-circle-bold">
        Add Project
      </Button>
      <Button variant="accent" icon="add-circle-bold" iconPosition="right">
        Add Project Right
      </Button>
      <Button variant="destructive" icon="trash-bin-trash-bold">
        Delete
      </Button>
