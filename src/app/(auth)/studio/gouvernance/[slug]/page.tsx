...
export default function EditProposalPage() {
  ...
  const proposal = useQuery(
    api.governance.getProposalBySlug,
    slug ? { slug } : "skip"
  );
  const updateProposal = useMutation(api.governance.updateProposal);
  ...
  const [voteDurationDays, setVoteDurationDays] = useState<number>(7);
  ...
  // Charger les données de proposal dès qu'elles sont disponibles
  useEffect(() => {
    if (proposal) {
      setTitle(proposal.title || "");
      setDescription(proposal.description || "");
      setProposalType(proposal.proposalType || "editorial_rules");
      setStatus(proposal.status || "draft");
      
      // Charger actionData - s'assurer que c'est bien un objet
      const loadedActionData = proposal.actionData && typeof proposal.actionData === 'object' 
        ? { ...proposal.actionData } 
        : {};
      setActionData(loadedActionData);
      
      if (proposal.voteEndAt && proposal.voteStartAt) {
        const durationMs = proposal.voteEndAt - proposal.voteStartAt;
        const durationDays = Math.ceil(durationMs / (1000 * 60 * 60 * 24));
        setVoteDurationDays(durationDays);
      } else if (currentVoteParams?.defaultDurationDays) {
        setVoteDurationDays(currentVoteParams.defaultDurationDays);
      }
    }
  }, [proposal, currentVoteParams]);
  ...
  // Vérifier que la proposition est en draft et que l'utilisateur est le proposant
  const canEdit = proposal && proposal.status !== "closed";

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    
    if (!proposal || !canEdit) {
      toast.error("Cette proposition ne peut pas être modifiée");
      return;
    }
    ...
    try {
      await updateProposal({
        ...
        title,
        slug: generatedSlug !== proposal.slug ? generatedSlug : undefined,
        description,
        proposalType,
        voteDurationDays,
        status: status !== proposal.status ? status : undefined,
      ...
      toast.error(error.message || "Erreur lors de la mise à jour de la proposition");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!slug) {
    return (
      <div className="flex items-center justify-center h-64">
        <p className="text-muted-foreground">Slug manquant</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          <SolarIcon icon="spinner-circle" className="h-8 w-8 mx-auto animate-spin text-muted-foreground mb-2" />
  ...
  if (proposal === null) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          <SolarIcon icon="danger-triangle-bold" className="h-8 w-8 mx-auto text-destructive mb-2" />
          <p className="text-muted-foreground">Proposition introuvable</p>
          <Button onClick={() => router.push("/studio/gouvernance")} className="mt-4">
            Retour
          </Button>
        </div>
      </div>
  ...
  if (!canEdit) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          <SolarIcon icon="lock-bold" className="h-8 w-8 mx-auto text-destructive mb-2" />
          <p className="text-muted-foreground mb-2">
            Cette proposition ne peut pas être modifiée
          </p>
          <p className="text-sm text-muted-foreground mb-4">
            {proposal.status === "closed"
              ? "Les propositions fermées ne peuvent pas être modifiées"
              : "Vous n'êtes pas autorisé à modifier cette proposition"}
          </p>
          <Button onClick={() => router.push("/studio/gouvernance")} variant="outline">
            Retour
          </Button>
        </div>
      </div>
      ...
      <div className="px-3 py-3 border-b shrink-0 bg-background">
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground">
            Modifiez votre proposition de gouvernance
          </p>
        </div>
      </div>
      ...
      <ScrollArea className="flex-1 min-h-0">
        <div className="px-3 py-3 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="open" className="text-xs">Vote ouvert</SelectItem>
                <SelectItem value="closed" className="text-xs" disabled>Vote fermé (non modifiable)</SelectItem>
              </SelectContent>
            </Select>
            <p className="text-[10px] text-muted-foreground">
              {status === "draft" && "La proposition n'est pas encore publiée"}
              {status === "open" && "La proposition est ouverte au vote"}
              {status === "closed" && "La proposition est fermée et ne peut plus être modifiée"}
            </p>
          </div>
          ...
          {(() => {
            ...
            // S'assurer que les règles sont chargées avant d'afficher les champs
            if ((proposalType === "editorial_rules" || proposalType === "product_evolution") && availableRules === undefined) {
              return (
            ...
            return (
              <ActionFields
                key={JSON.stringify(actionData)} // Force re-render quand actionData change
                schema={schema}
          ...
          {generatedSlug && (
            <>
              ...
              <div className="space-y-1">
                ...
                {generatedSlug !== proposal.slug && (
                  <p className="text-[10px] text-muted-foreground">
                    L'URL changera lors de la sauvegarde
                  </p>
                )}
              </div>
            </>
