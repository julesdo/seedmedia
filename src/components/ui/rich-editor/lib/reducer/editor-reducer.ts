...
/**
 * Vérifie si un bloc est éditable (pas un bloc média ou link preview)
 */
function isEditableBlock(node: EditorNode): boolean {
  if (!isTextNode(node)) {
    return false // Les containers ne sont pas directement éditables
  }
  
  // Les blocs média ne sont pas éditables
  if (node.type === "img" || node.type === "video" || node.type === "chart") {
    return false
  }
  
  // Vérifier si c'est un link preview (un seul enfant avec href et pas de contenu texte)
  if (node.children && node.children.length === 1 && node.children[0]?.href) {
    const linkChild = node.children[0]
    const isLinkPreview = 
      !linkChild.content?.trim() || 
      linkChild.content?.trim() === linkChild.href ||
      linkChild.content?.trim() === linkChild.href?.replace(/^https?:\/\//, "")
...
/**
 * S'assure qu'il y a toujours au moins un bloc éditable à la fin du conteneur
 */
function ensureEditableBlockAtEnd(container: ContainerNode): ContainerNode {
  const lastChild = container.children[container.children.length - 1]
  
  if (!lastChild || !isEditableBlock(lastChild)) {
    const timestamp = Date.now()
    const editableBlock: TextNode = {
      id: `p-${timestamp}`,
      type: "p",
      content: "",
      attributes: {},
    ...
    return {
      ...container,
      children: [...container.children, editableBlock],
    }
  }
...
export function editorReducer(
  ...
  switch (action.type) {
    case "UPDATE_NODE": {
      ...
      let newContainer = updateNodeById(
        currentContainer,
        id,
        () => updates
      ) as ContainerNode

      // Vérifier si le bloc mis à jour est devenu non-éditable (link preview, etc.)
      const updatedNode = findNodeById(newContainer, id)
      if (updatedNode && !isEditableBlock(updatedNode)) {
        // Trouver l'index du bloc mis à jour
        const nodeIndex = newContainer.children.findIndex(child => child.id === id)
        
        if (nodeIndex !== -1) {
          // Vérifier s'il y a déjà un bloc éditable après
          const nextNode = newContainer.children[nodeIndex + 1]
          const hasEditableAfter = nextNode && isEditableBlock(nextNode)
          ...
          if (!hasEditableAfter) {
            ...
            // Insérer après le bloc non-éditable
            newContainer = insertNode(
              newContainer,
              id,
              editableBlock,
              "after"
      ...
      // S'assurer qu'il y a toujours au moins un bloc éditable à la fin du conteneur
      newContainer = ensureEditableBlockAtEnd(newContainer)
    ...
    case "DELETE_NODE": {
      ...
      console.log("🔍 DELETE_NODE reducer: id à supprimer:", id, "container actuel a", currentContainer.children.length, "enfants")
      const result = deleteNodeById(currentContainer, id)
      console.log("🔍 DELETE_NODE reducer: résultat", result ? `container avec ${(result as ContainerNode).children.length} enfants` : "null")
      ...
      let resultContainer = result as ContainerNode
      ...
      // S'assurer qu'il y a toujours au moins un bloc éditable à la fin après la suppression
      resultContainer = ensureEditableBlockAtEnd(resultContainer)
    ...
    case "INSERT_NODE": {
      ...
      // Si c'est un bloc non-éditable inséré après un autre bloc, ajouter un bloc éditable après
      if (!isEditableBlock(node) && position === "after") {
        const timestamp = Date.now()
        const editableBlock: TextNode = {
          id: `p-${timestamp}`,
          type: "p",
        ...
        // Insérer le bloc éditable après le bloc non-éditable qu'on vient d'insérer
        newContainer = insertNode(
          newContainer,
          node.id,
          editableBlock,
    ...
    case "REPLACE_CONTAINER": {
      ...
      // S'assurer qu'il y a toujours au moins un bloc éditable à la fin
      const ensuredContainer = ensureEditableBlockAtEnd(container)
...
export function createInitialState(
  ...
  let initialContainer: ContainerNode = {
    id: "root",
    type: "container",
  ...
  // S'assurer qu'il y a toujours au moins un bloc éditable à la fin
  initialContainer = ensureEditableBlockAtEnd(initialContainer)
