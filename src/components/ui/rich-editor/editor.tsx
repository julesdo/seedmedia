...
EditorProps = {}) {
  ...
  // Utiliser useRef pour stocker activeNodeId et éviter les problèmes de dépendances
      ...
      if (element) {
        ...
        if (isFocusable && document.activeElement !== element) {
          try {
            ...
            // Ignorer les erreurs de focus (peut arriver si l'élément n'est pas encore dans le DOM)
          }
        } else if (!isFocusable) {
          // Si l'élément n'est pas focusable (comme une image), c'est normal, pas d'erreur
          return
        }
      } else if (retries < 10) {
        setTimeout(() => attemptFocus(retries + 1), 50)
      } else {
        // Vérifier si c'est un node container (comme une image) - c'est normal qu'il ne soit pas focusable
        // On utilise useEditorStore.getState() pour éviter d'ajouter container aux dépendances
        try {
          const state = useEditorStore.getState()
          const currentContainer = state.history[state.historyIndex]
          const node = currentContainer.children.find((n: any) => n.id === activeId) || 
                      (currentContainer.id === activeId ? currentContainer : null)
          
          if (node && ('type' in node) && (node.type === 'img' || node.type === 'video' || node.type === 'table' || node.type === 'flex')) {
            // C'est un node container, pas besoin de focus
            return
          }
        } catch (err) {
          // Ignorer les erreurs de récupération du state
        }
        
        // Sinon, c'est une vraie erreur
        console.warn(
          "⚠️ [Focus] Element not found after 10 retries (may be a container):",
          activeId
        )
      }
    }
            ...
            <div ref={editorContentRef} className="h-full">
              <>
                ...
                >
                  {container.children
                    .filter((node) => {
                      // Filter out free-positioned images BEFORE mapping to ensure consistent hook count
                    ...
                    .map((node, filteredIndex) => {
                      // Calculate original index for isFirstBlock check
                      const originalIndex = container.children.findIndex(
                        (n) => n.id === node.id
                      )
                      ...
                      const isFirstBlock = originalIndex === 0
                      const isSecondBlock = originalIndex === 1

                      // Re-check if this is a text node (needed for nodeRef callback)
                      const isText = isTextNode(node)
                      const textNode = isText ? (node as TextNode) : null
