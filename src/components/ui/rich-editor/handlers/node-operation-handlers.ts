...
export function createHandleDeleteNode(
  ...
  return (nodeId: string) => {
    ...
    console.log("🗑️ createHandleDeleteNode appelé avec nodeId:", nodeId)
    ...
    console.log("🔍 Node trouvé pour suppression:", nodeToDelete ? nodeToDelete.id : "NON TROUVÉ")
    ...
    console.log("🔍 Parent container trouvé:", parentContainer ? (parentContainer as ContainerNode).id : "AUCUN")
    ...
    if (parentContainer) {
      ...
      console.log("📊 Enfants restants dans le container:", remainingChildren.length, "sur", containerNode.children.length)

      // If only one child left, unwrap it from the container
      if (remainingChildren.length === 1) {
        console.log("✅ Un seul enfant restant, déballage du container")
        ...
        console.log("🚀 Dispatch batch actions pour déballage")
        dispatch(EditorActions.batch(actions))
      } else if (remainingChildren.length === 0) {
        // No children left, delete the container
        console.log("🚀 Suppression du container (aucun enfant restant)")
        dispatch(EditorActions.deleteNode(parentContainer.id))
      } else {
        // Multiple children remain - update the container directly instead of using deleteNodeById
        console.log("🚀 Mise à jour directe du container (plusieurs enfants restants)")
        // Update the container to remove the node
        dispatch({
          type: "UPDATE_NODE",
          payload: {
            id: containerNode.id,
      ...
      console.log("🚀 Suppression du node (pas dans un container)")
