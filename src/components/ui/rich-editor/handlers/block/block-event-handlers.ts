...
export function createHandleKeyDown(params: BlockEventHandlerParams) {
  return (e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    // Détection automatique des URLs après Espace ou Entrée
    if ((e.key === " " || e.key === "Enter") && !e.shiftKey && !isListItem) {
      const element = e.currentTarget
      const text = element.textContent || ""
      ...
      if (text.trim()) {
        ...
        if (matches && matches.length > 0) {
          // Vérifier si le block contient uniquement une URL (pas de texte avant/après)
          const url = matches[0]
          const urlIndex = text.indexOf(url)
          const beforeUrl = text.substring(0, urlIndex).trim()
          const afterUrl = text.substring(urlIndex + url.length).trim()
          
          // Si le block contient uniquement l'URL, créer un block link
          if ((beforeUrl === "" && afterUrl === "") || text.trim() === url.trim()) {
            // Normaliser l'URL
            ...
            // Déférer la transformation pour éviter les problèmes de hooks
            setTimeout(() => {
              dispatch({
                type: "UPDATE_NODE",
                payload: {
            ...
            // Ne pas continuer avec la touche normale
            if (e.key === "Enter") {
              e.preventDefault()
            }
            return
