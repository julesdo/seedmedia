...
export const Block = React.memo(
  function Block({
    ...
    // If node not found, return a hidden placeholder (don't return null to maintain hook count)
    if (!node) {
          ...
          >
            {containerNode.children.map((childNode) => {
              const isChildMedia =
                childNode &&
                "type" in childNode &&
                (childNode.type === "img" || childNode.type === "video" || childNode.type === "chart")
              ...
              return (
                <div
                  key={childNode.id}
                  className="relative max-w-full min-w-[280px] flex-1"
                >
                  {blockContent}
                </div>
              )
    ...
    const commonProps = {
      ...
      className: `!ml-6
        ...
        ${!readOnly ? "hover:bg-accent/5 hover:shadow-sm" : ""}
    ...
    // Vérifier si c'est un block link (après tous les hooks pour éviter les problèmes)
    const hasSingleLinkChild =
      textNode.children &&
      textNode.children.length === 1 &&
      textNode.children[0] &&
      textNode.children[0].href
    const linkChild = hasSingleLinkChild ? textNode.children[0] : null
    const isOnlyLink =
      linkChild &&
    ...
    // Si c'est un block link, rendre LinkBlock
    if (hasSingleLinkChild && isOnlyLink) {
      return (
        <LinkBlock
          node={textNode}
          isActive={isActive}
    ...
    // Si c'est un block chart, rendre ChartBlock (après tous les hooks)
    if (textNode.type === "chart") {
      return (
        <ChartBlock
          node={textNode}
          isActive={isActive}
    ...
    // Si c'est un block code/pre, rendre CodeBlock (après tous les hooks)
    if (textNode.type === "code" || textNode.type === "pre") {
      return (
        <CodeBlock
          node={textNode}
          isActive={isActive}
        ...
        >
          ...
          >
            ...
            {!readOnly && onBlockDragStart && (
              <div className="mb-1 flex items-center gap-0.5 transition-opacity duration-300 ease-in-out md:absolute md:top-1/2 md:left-0 md:mb-0 md:-ml-[2rem] md:-translate-y-1/2 md:opacity-0 md:group-hover:opacity-100">
                ...
                {notionBased &&
                  ...
                  onUploadCoverImage && (
                    <>
                      ...
                      />
                      <button
                        className="hover:bg-accent rounded p-0.5 transition-colors duration-300 ease-in-out"
                        onClick={(e) => {
                          e.stopPropagation()
                      ...
                      >
                        {isUploadingCover ? (
                          ...
                          <ImageIcon
                            className="text-muted-foreground hover:text-foreground h-5 w-5 transition-colors duration-300 ease-in-out"
                            strokeWidth={1.5}
                          />
                        )}
                      </button>
