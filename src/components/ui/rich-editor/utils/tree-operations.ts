...
export function deleteNodeById(
  ...
  console.log("🔍 deleteNodeById appelé avec node.id:", node.id, "targetId:", targetId)
  
  // If this is the target node, signal deletion
  if (node.id === targetId) {
    console.log("✅ Node trouvé et marqué pour suppression:", node.id)
    return null
  }
  ...
  if (isContainerNode(node) || isStructuralNode(node)) {
    ...
    if (newChildren.length !== node.children.length) {
      console.log("✅ deleteNodeById: enfants modifiés", node.children.length, "->", newChildren.length, "pour node", node.id)
      return {
        ...node,
        children: newChildren,
      }
    } else {
      console.log("⚠️ deleteNodeById: aucun changement pour node", node.id, "(enfants:", node.children.length, ")")
    }
  }
