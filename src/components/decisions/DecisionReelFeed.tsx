...
export function DecisionReelFeed({
  ...
  const previousDecisionIdsRef = useRef<string>('');
  
  useEffect(() => {
    if (enrichedDecisions.length === 0) return;
    
    // Comparer les IDs pour éviter les mises à jour inutiles
    const currentIds = enrichedDecisions.map(d => d._id).join(',');
    if (currentIds === previousDecisionIdsRef.current) return;
    
    previousDecisionIdsRef.current = currentIds;
    ...
    setAllLoadedDecisions((prev) => {
      ...
      // Ne mettre à jour que s'il y a vraiment de nouvelles décisions
