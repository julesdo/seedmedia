...
## Convex Reactivity - Important Note

**⚠️ CRITICAL: Convex queries are reactive by default - NO useEffect needed!**

With Convex, `useQuery` automatically updates when data changes in the database. You **do NOT need** `useEffect` to reload data when:
- Switching tabs (Convex updates automatically)
- Data changes occur (Convex pushes updates)
- Component remounts (Convex handles caching)

**✅ Correct Pattern:**
```typescript
const data = useQuery(api.module.getData, { id });
...
useEffect(() => {
  // DON'T manually refetch - Convex does this automatically!
  refetchData().then(setData);
}, [dependency]);
```

**When useEffect IS appropriate:**
- Form state synchronization (syncing props to local state)
- UI effects (scroll, animation, IntersectionObserver)
- localStorage/sessionStorage synchronization
- Event listeners (not related to Convex data)
